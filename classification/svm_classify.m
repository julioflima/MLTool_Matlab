function [OUT] = svm_classify(DATA,PAR)

% --- SVM classifier Test ---
%
%   [OUT] = svm_classify(DATA,PAR)
%
%   Input:
%       DATA.
%           input = test data attributes [p x N]
%           output = test data labels [c x N]
%       PAR.
%           P = training data attributes [p x N]
%           T1 = training data labels [c x N]
...
for i = 1:Nts,
    ...
    % update kernel
    for j = 1:Ntr,
        K_ts1(j,1) = exp(-norm(Q(i,:)-P(j,:)).^2/sig2);
