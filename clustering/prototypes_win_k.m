function [c] = prototypes_win_k(C,sample,PAR)
...
elseif(length(size(C)) == 3),
    ...
    for i = 1:k1,
        for j = 1:k2,
            ...
            elseif (dist == 2)
                if kernel_type == 1,
                    Mdist(i,j) = 2 - 2*exp(-sum((sample - neuron).^2) / (2*sig2));
                elseif kernel_type == 2,
                    Mdist(i,j) = (2*sig2)/(sig2+1) + (2*sig2)/(sig2+sum((sample - neuron).^2));
                elseif kernel_type == 3,
                    Mdist(i,j) = -2*log(1+(sum((sample - neuron).^2)/sig2));
                end
            end
